...
from .method_model import all_method_item_ids, iter_method_variants
...
class CollectionStats:
    def __init__(self) -> None:
        self.timeseries_requested = 0; self.timeseries_succeeded = 0; self.timeseries_failed = 0; self.warnings: list[str] = []


class SeriesCollector:
    def __init__(self, client: MarketApiClient, stats: CollectionStats) -> None:
        self.client = client; self.stats = stats; self.cache: dict[tuple[int, str], list[TimeSeriesPoint]] = {}; self.failed_keys: set[tuple[int, str]] = set()
    ...
    def get(self, item_id: int, timestep: str) -> list[TimeSeriesPoint]:
        ...
        try:
            ...
            self.stats.timeseries_failed += 1; self.failed_keys.add(key)
            warning = f"TIMESERIES_FAILED item={item_id} timestep={timestep}: {exc}"
            self.stats.warnings.append(warning); LOGGER.warning(warning); series = []
        else: self.stats.timeseries_succeeded += 1
...
def _record_for_item(item: MappingItem, latest: LatestPrice, windows: dict[str, dict[str, Any]], generated_at: int, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"schemaVersion": SCHEMA_VERSION, "generatedAt": generated_at, "item": item.to_dict(), "latest": latest.to_api_dict(), "current": current, "windows": windows, "quality": build_quality(current, windows, item.limit, item.highalch, settings)}


def _mapping_name_index(mapping: dict[int, MappingItem]) -> dict[str, list[int]]:
    index: dict[str, list[int]] = {}
    for item_id, item in mapping.items(): index.setdefault(item.name.casefold(), []).append(item_id)
    return index


def _resolve_catalog_item_names(methods_config: dict[str, Any], mapping: dict[int, MappingItem]) -> None:
    """Resolve Wave 5 item_name entries against the live official price mapping.

    Resolution fails closed on missing or ambiguous names. This keeps catalogue
    source files readable while production still evaluates immutable numeric IDs.
    """
    ...
    for method_id, method in methods_config.get("methods", {}).items():
        ...
        for _, variant in iter_method_variants(str(method_id), method):
            for side in ("inputs", "outputs"):
                for entry in variant.get(side, []):
                    if entry.get("item_id") is not None: continue
        ...
        for side in ("inputs", "outputs"):
            for entry in method.get(side, []):
                name = str(entry.get("item_name") or "").strip()
                if entry.get("item_id") is None and name and len(index.get(name.casefold(), [])) == 1: entry["item_id"] = index[name.casefold()][0]
        for raw in method.get("variants") or []:
            ...
            for side in ("inputs", "outputs"):
                for entry in overrides.get(side, []):
                    name = str(entry.get("item_name") or "").strip()
                    if entry.get("item_id") is None and name and len(index.get(name.casefold(), [])) == 1: entry["item_id"] = index[name.casefold()][0]
    if errors: raise ValueError("catalogue item-name resolution failed: " + "; ".join(errors))


def _required_method_item_ids(methods_config: dict[str, Any]) -> set[int]:
    ids: set[int] = set()
    for method in methods_config.get("methods", {}).values():
        if method.get("enabled", True) is not False: ids.update(all_method_item_ids(method))
    return ids


def _refresh_plan(mode: str) -> dict[str, tuple[str, ...]]:
    short = {"5m": ("6h", "24h"), "1h": ("7d",)}; long = {"6h": ("30d",)}
    if mode == "short": return short
    if mode == "long": return long
    if mode == "full": return {**short, **long}
    return {}
...
def _refresh_history(item_ids: set[int], history: dict[str, Any], collector: SeriesCollector, generated_at: int, mode: str) -> None:
    for timestep, window_keys in _refresh_plan(mode).items():
        for item_id in sorted(item_ids):
            ...
            if (item_id, timestep) in collector.failed_keys: continue
            updates = {key: calculate_window_metrics(slice_window(points, generated_at, WINDOW_SPECS[key].duration_seconds), WINDOW_SPECS[key]) for key in window_keys}
            if updates: put_item_windows(history, item_id, updates)
...
def _load_or_fetch_mapping(client: MarketApiClient, cache_dir: Path, generated_at: int, force_refresh: bool) -> dict[int, MappingItem]:
    if not force_refresh:
        ...
        if cached is not None:
            mapping, cached_at = cached; LOGGER.info("mapping cache: %s items generatedAt=%s", len(mapping), cached_at); return mapping
    mapping = client.get_mapping(); save_mapping(cache_dir, mapping, generated_at); return mapping
...
def _write_internal_report(root: Path, generated_at: int, records: dict[int, dict[str, Any]], base_ids: set[int], tracked_ids: list[int], methods_config: dict[str, Any], afk_results: list[dict[str, Any]], alchemy_candidates: list[dict[str, Any]], preliminary_count: int, alchemy_assumptions: dict[str, Any], health: dict[str, Any]) -> None:
    ...
    (root / "market" / "items").mkdir(parents=True, exist_ok=True); (root / "afk").mkdir(parents=True, exist_ok=True); (root / "alchemy").mkdir(parents=True, exist_ok=True)
    market_items = []
    for item_id in sorted(base_ids):
        if (record := records.get(item_id)) is None: continue
        market_items.append(record)
        if item_id in tracked_ids: write_json(root / "market" / "items" / f"{item_id}.json", record)
    write_json(root / "health.json", health); write_json(root / "catalogue-gap.json", build_catalog_gap_report(methods_config.get("methods", {})))
...
def collect(config_dir: Path, output_dir: Path, cache_dir: Path, mode: str = "full") -> None:
    if mode not in COLLECTION_MODES: raise ValueError(f"unsupported collection mode: {mode}")
    settings = load_yaml(config_dir / "settings.yaml"); tracked_config = load_json(config_dir / "tracked_items.json"); methods_config = load_yaml(config_dir / "methods.yaml"); alchemy_overrides = load_json(config_dir / "alchemy_exclusions.json")
    generated_at = int(time.time()); stats = CollectionStats(); client = MarketApiClient(api_settings(settings))
    ...
    latest = client.get_latest(); exempt_ids, unresolved = load_and_resolve_exemptions(config_dir / "ge_tax_exemptions.json", mapping)
    ...
    if not use_fire_staff: base_ids.add(fire_id)
    preliminary = []
    if bool(settings["alchemy"].get("enabled", True)):
        preliminary = preliminary_scan(mapping, latest, generated_at, settings, {int(x) for x in alchemy_overrides.get("excludedItemIds", [])}, {int(x) for x in alchemy_overrides.get("forcedItemIds", [])})
    history = load_history(cache_dir); candidate_ids = {int(row["itemId"]) for row in preliminary}; collector = SeriesCollector(client, stats)
    ...
    health = {"schemaVersion": SCHEMA_VERSION, "generatedAt": generated_at, "mode": mode, "status": health_status, "api": {"mapping": "ok", "latest": "ok", "timeseriesRequested": stats.timeseries_requested, "timeseriesSucceeded": stats.timeseries_succeeded, "timeseriesFailed": stats.timeseries_failed}, "history": {"shortGeneratedAt": history.get("shortGeneratedAt"), "longGeneratedAt": history.get("longGeneratedAt")}, "warnings": stats.warnings}
    ...
    internal_dir = output_dir / "internal-report"; public_dir = output_dir / "public-site"
    ...
    write_public_site(public_dir, build_public_afk(generated_at, afk_results), build_public_alchemy(generated_at, alchemy_candidates, alchemy_assumptions), build_public_status(generated_at, health, short_history_generated_at=history.get("shortGeneratedAt"), long_history_generated_at=history.get("longGeneratedAt")))
    gap = build_catalog_gap_report(methods_config.get("methods", {})); LOGGER.info("AFK methods: %s; catalogue coverage: %s%%; missing families: %s", len(methods_config.get("methods", {})), gap["coveragePct"], gap["missingFamilyCount"])


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Collect OSRS market data and build public/private artifacts"); sub = parser.add_subparsers(dest="command", required=True)
    collect_parser = sub.add_parser("collect"); collect_parser.add_argument("--config", default="config"); collect_parser.add_argument("--output", default="build"); collect_parser.add_argument("--cache-dir", default=".market-cache"); collect_parser.add_argument("--mode", choices=COLLECTION_MODES, default="full")
    gap_parser = sub.add_parser("catalog-gap"); gap_parser.add_argument("--config", default="config"); gap_parser.add_argument("--output", default="build/catalogue-gap.json")
    args = parser.parse_args(argv); logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    try:
        if args.command == "collect": collect(Path(args.config), Path(args.output), Path(args.cache_dir), args.mode); return 0
        methods = load_yaml(Path(args.config) / "methods.yaml").get("methods", {}); report = build_catalog_gap_report(methods); write_json(Path(args.output), report); LOGGER.info("catalogue coverage: %s%%", report["coveragePct"]); return 0
    except (ApiError, ValueError, KeyError, OSError, json.JSONDecodeError) as exc:
        LOGGER.error("collection failed: %s", exc); return 1
