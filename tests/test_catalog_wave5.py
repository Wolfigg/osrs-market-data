...
def test_wave5_adds_requested_catalogue_families():
    ...
    for method_id in (
        "enchant_sapphire_ring", "enchant_zenyte_amulet", "charge_water_orb", "charge_air_orb",
        "make_teleport_tablet_standard_varrock", "make_teleport_tablet_ancient_ghorrock",
        "make_teleport_tablet_lunar_catherby", "make_teleport_tablet_arceuus_barrows",
        "make_prayer_potions_v2", "make_super_combat_potions_v2", "cook_probabilistic_shark",
        "gather_yew_logs", "gather_magic_logs", "gather_teak_logs", "gather_mahogany_logs",
        "gather_camphor_logs", "gather_mining_amethyst", "gather_fishing_anglerfish",
    ):
...
def test_jewellery_family_covers_all_gems_and_shapes():
    ...
    for gem in ("sapphire", "emerald", "ruby", "diamond", "dragonstone", "onyx", "zenyte"):
        for shape in ("ring", "necklace", "bracelet", "amulet"):
            assert f"enchant_{gem}_{shape}" in catalog
...
def test_prescription_goggles_expected_and_conservative_consumption():
    ...
    variants = dict(iter_method_variants("make_prayer_potions_v2", method))
    goggles = variants["make_prayer_potions_v2__prescription_goggles"]
    secondary = goggles["inputs"][1]
...
def test_cooking_probability_respects_level_location_gauntlets_and_cape():
    ...
    gauntlets_80 = cooking_success_probability(profile, 80, "range", True, False)
    hosidius_80 = cooking_success_probability(profile, 80, "hosidius_10", False, False)
    assert 0 < normal_80 < 1
    assert gauntlets_80 > normal_80
    assert hosidius_80 > normal_80
...
def test_catalog_gap_v2_reports_requested_families_covered():
    ...
    for key in ("enchant_jewellery", "teleport_tablets", "orb_charging", "potion_v2", "cooking_burns", "more_gathering", "forestry_outputs"):
